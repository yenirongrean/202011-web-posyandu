...
<body>
	...
	<div class="container">
		<h2>Tambah Data Perkembangan Balita</h2>
		<hr>
		...
		if(isset($_POST['submit'])){
			...
			$sql = mysqli_query($koneksi, "INSERT INTO perkembangan(idperkembangan, idbalita, berat_badan, tinggi_badan, lingkar_kepala, id_imunisasi)
				VALUES('$idperkembangan', '$idbalita', '$berat_badan', '$tinggi_badan', '$lingkar_kepala', '$id_imunisasi')") or die(mysqli_error($koneksi));
				if($sql){
					echo '<script>alert("Berhasil menambahkan data."); document.location="../index-perkembangan.php";</script>';
				}else{
		...
		<form action="create-perkembangan.php" method="post" autocomplete="off">
			<div class="form-group row">
				<label class="col-sm-2 col-form-label">ID Perkembangan</label>
				<div class="col-sm-10">
					<input type="text" name="idperkembangan" class="form-control" required>
				</div>
			</div>
			<div class="form-group row">
			...
			<div class="form-group row">
				...
				<div class="col-sm-10">
					<input type="text" name="id_imunisasi" class="form-control"
						placeholder="ID Imunisasi dapat dilihat pada menu Data Imunisasi">
				</div>
			</div>
